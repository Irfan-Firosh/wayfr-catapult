...
Copied from backend/pipelines/scene-bridge-mvp/backend/bridge.py and integrated into the main backend.
"""
...
def compute_scene_objects(
    ...
    Args:
        glb_bytes: GLB file from MapAnything reconstruction.
        npz_bytes: scene_data.npz from reconstruction (camera_poses,
            intrinsics, source_frame_indices, world_transform, depth_maps).
        detections_json: Parsed JSON from the GSAM2 video annotator.

    Returns:
        List of scene objects with track_id, label, centroid_3d,
        bbox_3d_min/max, confidence, n_observations.
    """
    points, _colors = load_glb_points(glb_bytes)
    n_points = len(points)
    ...
    world_transform = npz["world_transform"]
    ...
    for view_i in range(num_views):
        ...
        proj_u, proj_v, depth = _project_points_to_frame(points, pose, K, world_transform)
